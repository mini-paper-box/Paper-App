...
class NewPurchaseOrderFrame(tk.Frame):
    def __init__(self, master):
        ...
        self.data = pr().extract_files_info()
    ...
    def create_widgets(self):
        ...
        ttk.Button(toolbar, text="Move Files", command=self.move_files).pack(side=tk.RIGHT)
        # ttk.Button(toolbar, text="Email", command=self.toggle_column_panel).pack(side=tk.RIGHT, padx=5)
        ttk.Button(toolbar, text="Rename", command=self.rename).pack(side=tk.RIGHT, padx=5)
        ttk.Button(toolbar, text="Send PO", command=self.submit_po).pack(side=tk.RIGHT, padx=5)
    ...
    def populate_treeview(self, rows=None):
        ...
        if self.data:
            for d in self.data:
                rows.append(dict(zip(self.columns, [*d])))
        for row in rows:
            row['ordered_msf'] = f"{float(row['width']) * float(row['length']) / 144 * int(row['ordered_quantity']) / 1000:.3f}"
            self.tree.insert("", "end", values=[row[col] for col in self.tree['column']])
    ...
    def edit_cell(self, event):
        ...
        # if self.columns[col_index] == "ordered_msf":
        #     return
        if self.columns[col_index] == "file_name":
            file_name = str(self.tree.set(rowid, "file_name"))
        ...
        def on_edit(event):
            ...
            if self.columns[col_index] in ("width", "length", "ordered_quantity", "ordered_msf"):
                try:
                    if not self.columns[col_index] == "ordered_msf":
                        width = float(self.tree.set(rowid, "width"))
                        length = float(self.tree.set(rowid, "length"))
                        qty = float(self.tree.set(rowid, "ordered_quantity"))
                        msf = f"{((width * length / 144) * qty) / 1000:.3f}"
                        self.tree.set(rowid, "ordered_msf", msf)
                    elif self.columns[col_index] == "ordered_msf":
                        width = float(self.tree.set(rowid, "width"))
                        length = float(self.tree.set(rowid, "length"))
                        sqft = (width * length / 144)
                        msf = float(self.tree.set(rowid, "ordered_msf"))
                        qty = math.ceil((msf * 1000) / sqft)
                        self.tree.set(rowid, "ordered_quantity", qty)
        #     return
                except ValueError:
    ...
    def move_files(self):
        ...
        for item in self.tree.get_children():
            supplier = self.tree.item(item)['values'][0]
