...
class SchedulerMailer:
    ...
    def send_email(self, to, subject, data_rows, cc=None, attachments=None):
        """Send HTML email via Outlook with alternating colors and chain start/end under leadtime."""
        html = f"""
        <!DOCTYPE html>
        <html>
        <head>
                        ...
                        <table width="800" cellpadding="0" cellspacing="0" border="0" style="background-color:#ffffff; border-radius:8px; overflow:hidden;">
                            ...
                            <tr>
                                <td style="padding:20px;">
                                    <p style="margin:0 0 20px 0;">The purpose of this timeline standard is to eliminate back and forth between sales and order management, allowing order management to respond quickly with order due dates.</p>
        ...
        for i, row in enumerate(data_rows):
            ...
            process_name, leadtime, ship_date, chain_start, chain_end = row
            html += f"""
                <tr>
                    <td style="border:1px solid #ddd; background-color:{bg_color};">{process_name}</td>
                    <td style="border:1px solid #ddd; background-color:{bg_color};">
                        {leadtime}<br/><small>({chain_start} → {chain_end})</small>
                    </td>
                    <td style="border:1px solid #ddd; background-color:{bg_color};">{ship_date}</td>
                </tr>
            """
...
if __name__ == "__main__":
    ...
    nozomi = 10
    schedules_to_run = {
        "Elitron": (msf, [13], max_days, elitron, 1),
        "Eterna": (msf*2, [3], max_days, eterna, 1),
        "Langston": (msf*4, [1], max_days, langston, 1),
        "United": (msf*4, [2], max_days, united, 1),
        "Elitron_Glue": (msf, [13,15], max_days, elitron, 1),
        "Eterna_Langston": (msf*3, [3,1], max_days, eterna, 1),
        "United_Langston": (msf*4, [2,1], max_days, united, 1),
        "Nozomi": (msf*4, [6], max_days, nozomi, 1),
        "Nozomi_Elitron": (msf, [6,13], max_days, nozomi+1, 1),
        "Nozomi_Eterna": (msf*2, [6,3], max_days, nozomi, 1),
        "Nozomi_United": (msf*5, [6,2], max_days, nozomi, 1),
        "Nozomi_Langston": (msf*5, [6,1], max_days, nozomi, 1),
        "Nozomi_Eterna_Langston": (msf*2, [6,3,1], max_days, nozomi, 1),
        "Nozomi_United_Langston": (msf*5, [6,2,1], max_days, nozomi, 1)
    }
    ...
    for sch_name, sch in results.items():
        ...
        chain_start = sch[0][7]  # Updated index for chain_start_date
        chain_end = sch[0][8]    # Updated index for chain_end_date
        ...
        email_rows.append((process_names, f"{lead_days} workdays", ship_date.strftime("%Y-%m-%d"), chain_start, chain_end))
