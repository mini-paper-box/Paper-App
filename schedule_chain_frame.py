...
class SchedulerChainFrame(tb.Frame):
    def __init__(self, master, db_path="prod_db.db"):
        ...
        proc_frame.columnconfigure(0, weight=3)  # Left list
        proc_frame.columnconfigure(1, weight=1)  # Buttons
        proc_frame.columnconfigure(2, weight=3)  # Right list
        ...
        tb.Label(proc_frame, text="Selected Sequence").grid(row=0, column=2, sticky='ns')
        self.selected_listbox = tk.Listbox(proc_frame, selectmode=tk.SINGLE, height=10, exportselection=False)
        self.selected_listbox.grid(row=1, column=2, sticky="nsew", padx=5, pady=5)


        # Buttons for add/remove
        btn_frame = tb.Frame(proc_frame)
        btn_frame.grid(row=1, column=1, sticky=W, padx=5, pady=5)
    ...
    def enable_drag_and_drop(self, listbox):
        ...
        listbox.bind("<Button-2>", self.add_process)
