...
class SQLManager:
    ...
    def fetch_order_routing(self, order_id, qty):
        ...
        query = """
            WITH routing AS (
                SELECT 
                    ...
                    p.process_nme,
                    oor.order_seq,
                    CAST(od.order_qty AS FLOAT) * (d.sqfpm / 1000.0) AS run_sqft,
                    d.sqfpm,
        ...
        try:
            ...
            #     # Parameters: 1st is Qty, 2nd is Docket_ID
            #     # return pd.read_sql(sql, conn, params=(qty, str(docket_id)))
            return self.safe_fetch(query,params=(str(order_id)))
        except Exception as e:
            print(f"Routing Query Error: {e}")
            return pd.DataFrame(columns=['seq_order', 'process_id', 'process_name', 'run_sqft'])
    ...
    def fetch_shipment_report(self):
        query = """
            ...
            SELECT 
                ...
                oh.ship_add1,
                CONCAT(oh.ship_city, ' ', oh.ship_prov) AS [ship_city],
            ...
            LEFT JOIN unitizing_skid_type ust  
                ON d.unitizing_skid_type_id = ust.skid_type_id

            LEFT JOIN customer_shipto cst
                ON oh.customer_id = cst.customer_id
                AND oh.ship_nbr = cst.ship_nbr
    ...
    def fetch_shipment_report_usa(self):
        query = """
            WITH Dates AS (
                SELECT
                    CAST(GETDATE() AS DATE) AS today,
