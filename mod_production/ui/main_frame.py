...
class ProductionPlannerFrame(tb.Frame):
    ...
    def run_scheduler(self):
        ...
        try:
            ...
            # Get metadata docket routing etc
        ...
        try:
            ...
            try:
                ...
                if hasattr(self.ai_engine, 'predict_batch'):
                    ...
                    #add predict mins to processed df
            ...
            self.booked_df = processed_df.groupby(
                ['process_id', 'schedule_date'],
                as_index=False
            ).agg(
                # mins_booked=('total_time', 'sum'),
                mins_booked=('total_m', 'sum'),
                booked_sqft=('booked_sqft', 'sum'),
                booked_jobs=('process_id', 'count')
            )
            print(processed_df[processed_df["full_path"] == "69"])
    ...
    def handle_retrain(self):
        ...
        try:
            ...
            df_filtered = df_filtered[df_filtered['job_qty'] > 0]
    ...
    def predict_only(self, docket_id, qty):
        """
        Fast predictor call.
        Returns only routing + AI prediction data.
        """
