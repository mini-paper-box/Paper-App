...
if __name__ == "__main__":
    ...
    # Define schedules to run: (job_size, process_ids)
    msf = 10000
    schedules_to_run = {
        "Elitron": (msf, [231]),
        "Eterna": (msf, [3]),
        "Langston": (msf, [1]),
        "United": (msf, [2]),
        "Elitron_Glue": (msf, [231,45]),
        "Eterna_Langston": (msf, [3,1]),
        "United_Langston": (msf, [2,1]),
        "Nozomi": (msf, [6]),
        "Nozomi_Elitron": (msf, [6,13]),
        "Nozomi_Eterna": (msf, [6,3]),
        "Nozomi_United": (msf, [6,2]),
        "Nozomi_Eterna_Langston": (msf, [6,3,1]),
        "Nozomi_United_Langston": (msf, [6,2,1])
    }

    results = {}
    for name, (size, pids) in schedules_to_run.items():
        results[name] = scheduler.run_schedule(size, pids)
