...
class AutoScheduler:
    ...
    def fetch_capacity_matrix(self):
        """Build P80 capacity matrix by process and job count"""
        query = """
        SELECT t.process_id, 
               CAST(t.start_date AS DATE) as work_date,
               COUNT(t.docket_id) as job_count,
        ...
        # Calculate P80 capacity for each process/job_count combination
        matrix = raw_hist.groupby(['process_id', 'job_count'])['daily_msf'].quantile(0.8).reset_index()
        matrix.rename(columns={'daily_msf': 'p80_capacity'}, inplace=True)
        return matrix

    def fetch_docket_routing(self, docket_id, run_qty):
        query = """
            SELECT dr.sequence AS seq_order, dr.process_id, dr.routing_dsc AS process_name,
        ...
        """
        return pd.read_sql(query, self.connect(), params=[run_qty, docket_id])
...
class SchedulerGUI:
    def __init__(self, root):
        ...
        # --- TOP PANELS (Parameters, Style, History) ---
        ...
        # 2. STYLE (Dynamic stats based on docket)
        ...
        self.lbl_chart = tb.Label(chart_nav, text="Process Capacity View", 
                                  font=("Helvetica", 11, "bold"), bootstyle="inverse-info")
    ...
    def update_chart(self):
        """High-detail bar chart with 25-day view"""
        ...
        if self.capacity_matrix is not None:
            ...
            if not proc_matrix.empty:
                matrix_text = "Capacity Matrix 80 Percentile:\n"
        ...
        daily_vals = pd.Series(0, index=dates)
        ...
        if self.booked_df is not None and not self.booked_df.empty:
            ...
            for _, row in p_booked.iterrows():
                if row['schedule_date'] in daily_vals.index:
                    daily_vals[row['schedule_date']] = row['booked_sqft']
                    daily_jobs[row['schedule_date']] = row['booked_jobs']

        x = range(len(dates))
        bars = self.ax.bar(x, daily_vals.values, color='#3498db', alpha=0.7, 
                          edgecolor='#2980b9', linewidth=1.5, label='Existing Bookings')
        
        # Highlight scheduled dates in Red
        if proc['start'] and proc['end']:
            for i, d in enumerate(dates):
                if proc['start'] <= d <= proc['end']:
        ...
        # Add text labels on bars showing sqft and job count
        proc_tier_usage = self.day_tier_usage.get(proc['id'], {})
        
        for i, (d, sqft, jobs) in enumerate(zip(dates, daily_vals.values, daily_jobs.values)):
            if sqft > 0:
                # Get tier used for this day if it was scheduled
                tier_used = proc_tier_usage.get(d)
                
                # Show sqft/jobs on bar
                label_text = f'{sqft:,.0f}/{int(jobs)}'
                if tier_used:
                    label_text += f' [T{tier_used}]'
                
                self.ax.text(i, sqft + (self.ax.get_ylim()[1] * 0.02), 
                           label_text, ha='center', va='bottom', 
                           color='#f39c12' if tier_used else '#adb5bd', 
                           fontsize=7, fontweight='bold')
                
                # Show job count inside bar
                if jobs > 0:
                    self.ax.text(i, sqft * 0.5, 
                               f'{int(jobs)}', ha='center', va='center', 
        ...
        # Summary stats
        total_booked = daily_vals.sum()
        total_jobs = daily_jobs.sum()
        summary = f"Total Booked: {total_booked:,.0f} sqft | {int(total_jobs)} jobs | Required: {proc['sqft']:,.0f} sqft"
        self.ax.text(0.98, 0.98, summary, transform=self.ax.transAxes,
                    fontsize=9, color='#adb5bd', ha='right', va='top',
    ...
    def run_scheduler(self):
        ...
        try:
            ...
            self.capacity_matrix = matrix  # Store for display
            ...
            # 3. Build schedule using P80 bucket logic
            self.process_data = []
            ...
            for _, prow in routing.iterrows():
                ...
                # Initialize tier tracking for this process
                if pid not in self.day_tier_usage:
                    self.day_tier_usage[pid] = {}
                
                # If bypassed, just add 1 day after previous end
                if pid in ("40","237","238","239","240","241","242","245","193","243","235"):
                    ...
                    for d in workdays:
                        ...
                        # Find P80 capacity for this job count
                        ...
                        # If no exact match, use minimum P80 for this process
                        if cap_match.empty:
                        ...
                        # Track which tier was used for this day
